...
class YOLOWorldDetector(YOLODetector):
    ...
    def reparameterize(self, texts: List[List[str]]) -> None:
        ...
        self.texts = texts
        self.text_feats, text_masks = self.backbone.forward_text(texts)
        # Persist masks so cached embeddings can be reused safely
    ...
    def extract_feat(
        ...
        if txt_feats is not None:
            ...
            if txt_masks is None:
                raise RuntimeError(
                    "Cached text masks missing. Call reparameterize first."
                )
        else:
